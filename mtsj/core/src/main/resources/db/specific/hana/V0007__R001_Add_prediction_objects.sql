...
for i in range(7):
    dishes[i] = dishes[i].reindex(index)
    dishes[i] = pd.merge(dishes[i]['amount'], dateinfo, left_index=True, right_index=True)
    dishes[i]['amount'] = dishes[i]['amount'].fillna(0) # nan
    dishes[i].loc[~dishes[i]['designation'].isnull(),'designation'] = 1  # not nan
    dishes[i]['designation'] = dishes[i]['designation'].fillna(0) # nan
    dishes[i].index = pd.DatetimeIndex(dishes[i].index.values,freq=dishes[i].index.inferred_freq)
    exog=dishes[i][['temperature', 'designation']].values
    sarimax_model = SARIMAX(endog=dishes[i]['amount'],order=models[i].order,seasonal_order=models[i].seasonal_order,exog=exog)
    res = sarimax_model.fit(disp=False)
    forecast= res.forecast(steps=7, exog=test_exog)
    print(forecast)
