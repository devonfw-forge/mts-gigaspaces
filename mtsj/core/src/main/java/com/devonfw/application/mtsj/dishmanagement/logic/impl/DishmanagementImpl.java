...
public class DishmanagementImpl extends AbstractComponentFacade implements Dishmanagement {
  ...
  @Override
  public CategoryEto findCategory(String id) {
  ...
  @Override
  public DishCto findDish(String id) {
  ...
  public Page<DishCto> findDishCtos(DishSearchCriteriaTo criteria) {
    ...
    for (DishEntity dish : searchResult.getContent()) {
      ...
      try {
        Blob dishContent = dish.getImage().getContent();
        cto.getImage()
            .setContent(Base64.getEncoder().encodeToString(dishContent.getBytes(1L, (int) dishContent.length())));
  ...
  private boolean dishAlreadyAdded(List<DishCto> dishEntitiesFiltered, DishCto dishEntity) {
    ...
    for (DishCto entity : dishEntitiesFiltered) {
      String entityId = entity.getDish().getId();
      String dishEntityId = dishEntity.getDish().getId();
  ...
  @Override
  public IngredientEto findIngredient(String id) {
